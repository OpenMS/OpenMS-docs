...
To to use another solver try `cmake ... -D USE_COINOR=1 ....` and look at the documentation of the `LPWrapper` class.
...
`Travis <https://travis-ci.org/>`_ is an automated system for continuous integration. Each new commit and pull request is automatically run through the travis build system.
